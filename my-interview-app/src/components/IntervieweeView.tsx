...
//   { difficulty: 'Easy', duration: 30 },   // First question
//   { difficulty: 'Medium', duration: 60 }, // Second question
// ];
...
//     setIsLoading(true);
//     setCurrentQuestion('');
//     const prompt = `Generate one ${difficulty}-level interview question for a full stack developer role focusing on React and Node.js. Provide only the question text, with no introductory phrases.`;
//     try {
...
//       const questionText = result.candidates?.[0]?.content?.parts?.[0]?.text.trim() || 'Could not generate a question. Please proceed.';
//       setCurrentQuestion(questionText);
//       setTimer(INTERVIEW_QUESTIONS_CONFIG[questionIndex].duration); // Start timer
//     } catch (err) {
...
//     }
//   };
//   // (extractDetailsWithAI function from previous step is omitted for brevity but should be kept)
//   const extractDetailsWithAI = async (text: string) => { /* ... keep previous implementation ... */ setIsExtracting(true); const prompt = `From the following resume text, extract the full name, email address, and phone number. Return the result as a JSON object with the keys "name", "email", and "phone". If a field is not found, its value should be null.\n\nText: "${text.substring(0, 4000)}"`; try { const apiKey = import.meta.env.VITE_GEMINI_API_KEY; if (!apiKey) { throw new Error("API key is not configured. Please add VITE_GEMINI_API_KEY to your .env.local file."); } const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=${apiKey}`; const payload = { contents: [{ parts: [{ text: prompt }] }] }; const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) throw new Error(`API call failed: ${response.status}`); const result = await response.json(); const rawJson = result.candidates?.[0]?.content?.parts?.[0]?.text || '{}'; const cleanedJson = rawJson.replace(/```json/g, '').replace(/```/g, '').trim(); const parsedDetails: CandidateDetails = JSON.parse(cleanedJson); setCandidateDetails(parsedDetails); const missing: string[] = []; if (!parsedDetails.name) missing.push('name'); if (!parsedDetails.email) missing.push('email'); if (!parsedDetails.phone) missing.push('phone'); if (missing.length > 0) { setStep('collectingInfo'); setMissingFields(missing); setChatMessages([{ sender: 'bot', text: "Thanks for uploading your resume." }]); } else { setStep('ready'); } } catch (err) { console.error("AI extraction error:", err); setError(err instanceof Error ? err.message : "Failed to extract details using AI."); } finally { setIsExtracting(false); } };
...
//     }
//   };
//   // (handleFileChange and handleChatSubmit are omitted for brevity but should be kept)
//   const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => { setError(''); setIsLoading(true); const file = event.target.files?.[0]; if (!file) { setIsLoading(false); return; } if (file.type !== 'application/pdf') { setError('Invalid file type.'); setIsLoading(false); return; } try { const arrayBuffer = await file.arrayBuffer(); const pdf: PDFDocumentProxy = await getDocument(arrayBuffer).promise; let fullText = ''; for (let i = 1; i <= pdf.numPages; i++) { const page = await pdf.getPage(i); const textContent = await page.getTextContent(); fullText += textContent.items.map(item => (item as TextItem).str).join(' ') + '\n'; } await extractDetailsWithAI(fullText); } catch (err) { console.error('PDF parsing error:', err); setError('Error parsing your PDF file.'); } finally { setIsLoading(false); } };
...
//   return (
//     <div>
//       {/* ... keep UI for 'upload', 'collectingInfo' steps ... */}
...
//           <button onClick={handleStartInterview} style={{ marginTop: '10px', padding: '10px 20px' }}>Start Interview</button>
//         </div>
//       )}
...
//           <p>Thank you for your time. Your results will be processed shortly.</p>
//           <h4>Your Answers:</h4>
//           <ul>
//             {interviewRecords.map((record, idx) => (
//               <li key={idx} style={{ marginBottom: '10px' }}>
//                 <strong>Q{idx + 1} ({record.difficulty}):</strong> {record.question}
//                 <br />
//                 <strong>Your Answer:</strong> {record.answer}
//               </li>
//             ))}
//           </ul>
//         </div>
